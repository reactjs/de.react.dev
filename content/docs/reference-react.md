...
Wir empfehlen, [JSX](/docs/introducing-jsx.html) zu verwenden, um zu beschreiben, wie die Benutzeroberfläche aussehen soll. Jedes JSX-Element ist einfach syntaktischer Zucker für das Aufrufen von [`React.createElement()`](#createelement). Wenn du JSX verwendest, rufst du die folgenden Methoden normalerweise nicht direkt auf:
...
Unter [React.Component API Referenz](/docs/react-component.html) gibt es eine Liste der Methoden und Eigenschaften, die sich auf die `React.Component` Basis-Klasse beziehen.
...
> Hinweis
>
> Das `shouldComponentUpdate()` von `React.PureComponent` vergleicht Objekte nur oberflächlich. Falls diese komplexe Datenstrukturen enthalten, könnte es für tiefer gelegene Unterschiede zu falschen Negativbefunden kommen. Benutze `PureComponent` nur dann, wenn Props und State einfach sind, oder verwende [`forceUpdate()`](/docs/react-component.html#forceupdate) wenn du weißt, dass tiefergelegene Datenstrukturen sich verändert haben. Oder überlege dir, [unveränderliche (immutable) Objekte](https://facebook.github.io/immutable-js/) zu verwenden, um das schnelle Vergleichen geschachtelter Daten zu erleichtern.
>
> Darüber hinaus überspringt das `shouldComponentUpdate()` von `React.PureComponent` den kompletten der Komponente untergeordneten Teilbaum. Es sollte also sichergestellt werden, dass alle Kind-Komponenten auch "pur" sind.
...
Standardmäßig vergleicht es komplexe Objekte im Props-Objet nur oberflächlich. Wenn du Kontrolle über den Vergleich haben möchtest, kannst du `React.memo` als zweites Argument eine benutzerdefinierte Vergleichsfunktion mitgeben.
...
Klont und gibt ein neues React-Element zurück, bei dem es `element` als Ausgangspunkt verwendet. Das resultierende Element hat die Props des ursprünglichen Elements, in die die neuen Props oberflächlich eingefügt werden. Neue Kinder ersetzen die existierenden Kinder. `key` und `ref` des ursprünglichen Elements bleiben erhalten.
...
Es schützt jedoch auch `ref`s. Das bedeutet, dass wenn ein Kind mit einer `ref` kommt, wird diese nicht versehentlich vom Vorfahren gestohlen. Es bekommt die gleiche `ref`, verknüpft mit dem neuen Element.
...
`React.forwardRef` erstellt eine React-Komponente, die das [ref](/docs/refs-and-the-dom.html)-Attribut, das sie erhält, an eine ihr in der Baumstruktur untergeordneten Komponente weitergibt. Diese Technik ist nicht sehr verbreitet, ist jedoch ein zwei Fällen besonders nützlich:
